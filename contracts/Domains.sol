...
contract Domains is ERC721URIStorage{
    ...
    constructor(string memory _tld) payable ERC721('MSA name service','MNS'){
        tld = _tld;
        console.log("%s, nameservice deployed",_tld);
    }

    //function to give us price of domain depending on length
    ...
    function register(string calldata name) public payable {
        //ensure its an unregistered one
        require(domains[name] == address(0));
        uint _price = price(name);
        require(msg.value>=_price,"Not enough matic paid");
        domains[name] = msg.sender;
        console.log("%s,has registered a new domain",msg.sender);
        ...
        bytes(
            ...
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the MSA name service", "image": "data:image/svg+xml;base64,',
